...
struct motorStruct // Структура для локального управления и сбора данных по моторам
{
  ...
  int flagCalc;              // Нужно ли делать расчет скорости ПИД регулятором и устанавливать эту скорость в таймер
  int position;              // Текущая позиция в импульсах
  int destination;           // Цель назначение в позиции в импульсах
  int speedNow;              // Текущая заданная скорость интервал между шагами в микросекундах
  int speedNeed;             // Текущая заданная скорость интервал между шагами в микросекундах
  float angleSpeed;          // Текущая угловая скорость в градусах в секунду
  float predAngle;           // Предыдущий угол
  float PError;              // Пропорциональная ошибка для ПИД регулятора, храним для следующего расчета
  float IError;              // Интегральная ошибка для ПИД регулятора, храним для следующего расчета
  uint64_t predTime;         // Предыдущее времяя расчета
  float deltaTime;           // Дельта времени расчета
  ...
  float predTargetAngle; // Предыдущий целевой угол, пришедший по SPI
  float filteredSpeed;   // Текущая отфильтрованная скорость
};
