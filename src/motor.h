...
float calcSpeedMotor(int num) // Расчет скорости для мотора. в градусах в секунду dps
{
    uint64_t timeNow = micros();
    float deltaTime = (timeNow - motor[num].predTime) / 1000000.0; // Находим разницу по времени от прыдыдущего расчета
    motor[num].predTime = timeNow;
    if (deltaTime <= 0.005)
        deltaTime = 0.005; // Защита от деления на ноль
    motor[num].deltaTime = deltaTime;
    DEBUG_PRINTF("num = %i ", num);
    DEBUG_PRINTF("deltaTime = %f ", deltaTime);
    ...
    DError = (PError - motor[num].PError) / (motor[num].deltaTime); // Дифференциалная ошибка
...
void setMotorSpeed(int num_, float speed_)
{
    ...
    if (speed_ == 0) // Теперь все наши действия зависят от скорости если скорость 0 то и мотор не крутится и не нужно ничего делать
    {
        ...
        motor[num_].flagCalc = 0; // Выключаем расчеты
        DEBUG_PRINTF("STOP.");
    }
...
{
    static uint32_t timeflagMotor = 0;
    if (timeflagMotor + 10 <= millis()) // Срабатывает каждые 10 милисекунд
    {
        timeflagMotor = millis();
        // DEBUG_PRINTF ("+\n");
        for (int i = 0; i < 4; i++)
        {
            if (motor[i].flagCalc)
