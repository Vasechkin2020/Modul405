...
void MX_TIM2_Init(void)
{
  ...
  __HAL_TIM_CLEAR_FLAG(&htim2, TIM_FLAG_UPDATE); // Очистка флага переполнения
}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  {
    overflow_count++; // Увеличиваем счётчик переполнений// Callback-функция, которая вызывается при переполнении счетчика TIM2 раз в 78 минут и решвет проблему переполнения счетчика
  }
