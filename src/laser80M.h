...
uint32_t laser80_calcDistance(uint8_t *rx_bufferUART, uint8_t len_); // Вычисление дистанции по полученному буферу или ошибка
...
void laser80_setResolution(uint16_t port_, uint8_t reso_);   // Установка разрешения измерения есди 1- то 1 мм, если 2 то 0,1 мм. Непонятно работает ли нет фактически и на чем сказывается (время измерения?)
...
uint32_t laser80_calcDistance(uint8_t *rx_bufferUART, uint8_t len_)
{
    float distance = 0;
    uint32_t dist;
...
void laser80_setRange(uint16_t port_, uint8_t range_)
{
...
void laser80_setResolution(uint16_t port_, uint8_t reso_)
{
